...
const Login = () => {
  ...
  useEffect(() => {
      ...
      script.onload = () => {
        if (!initGoogle()) {
          initTimer = setInterval(() => {
            if (initGoogle()) clearInterval(initTimer);
          }, 200);
        }
      };
      script.onerror = () => setError('Failed to load Google Sign-In. Please try email login.');
    ...
    return () => { if (initTimer) clearInterval(initTimer); };
  }, []);
  ...
  const handleGoogleSignIn = () => {
    ...
    try {
      ...
      setError('Google Sign-In failed to open. Please try again.');
    }
  };
        ...
        <Card className="w-full max-w-md shadow-2xl border-0">
          ...
          <CardContent>
            ...
            <Button
              ...
              className="w-full h-12 text-base font-semibold bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-2 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50"
            >
              {!googleReady ? (
                <div className="flex items-center space-x-2">
